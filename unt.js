...
function get韻基() {
  ...
  所有韻.some(e => {
    if (e.is三等 === is三等 && e.韻列表.includes(音韻地位.韻)) {
      let i = e.韻列表.split(分隔符).findIndex(e => { return e.includes(音韻地位.韻); });
...
function 音值to切韻擬音J(音節, is原版) {
  ...
  音節.韻核 = when([
    ['鍾韻', 'ʉɔ'],
    ['陽韻', (is`開口 或 重紐A類` ? 'ɨ' : 'ʉ') + (is原版 ? 'ɐ' : 音節.韻核)],
    ['清韻', 'iæ'],
    [is原版 && '江韻', 'œˤ'],
    [is原版 && '嚴凡韻 幫組', 'œ'],
    ['莊組 眞臻欣韻 開口', 'ɹ̩'],
...
function get音節() {
  ...
  if (is專業模式 && 音節.韻核.includes('ˤ')) {
    二等元音記號 = 選項.二等元音記號.split('◌')[1];
    音節.韻核 = 音節.韻核.replace('ˤ', is`端組 或 來母 庚韻` && 二等元音記號.includes('˞') ? '' : 二等元音記號);
