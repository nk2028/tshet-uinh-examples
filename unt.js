...
function get聲母() {
  return when([
    ...
    [選項.前部聲母非三等 === 'pˤ lˤ' && '(幫組 或 來母) 非 三等', {
      幫: 'pˤ', 滂: 'pˤʰ', 並: 'bˤ', 明: 'mˤ',
      來: 'lˤ',
    }],
...
function get韻基() {
  ...
  if (選項['j 韻尾去聲作 ɹ'] && is`脂微齊祭皆廢咍灰夬泰韻 去聲`) 韻尾 = 'ɹ';
  return [韻核, 韻尾];
}
...
function 調整鈍C介音(音節) {
  ...
  const 鈍C介音列表 = 選項['鈍 C 介音'].trim().split(' ').map(e => e.replace(/[開合唇非低]/g, ''));
