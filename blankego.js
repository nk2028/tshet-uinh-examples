 ...
 * - 本系統的音韻地位分析（基於 poem《廣韻字音表》）與原表有數十小韻在聲母、開合、等、重紐類等歸類有所不同，除個別外均不影響拼寫體系
 * - 旨韻（脂上聲）若干精、章組小韻，韻母寫為B類，與他處精章組均用A類不合，現修正
 * - 崇開三眞上（濜）：真諄上去聲莊組開口，原表及拼寫均析為臻韻上去聲，惟此小韻遺漏，現修正
 * - 莊合三真入（𠭴）：韻母寫為A類，與他處莊組均用B類不合，現修正
 * - 以開一寒入（䔾）：原表作明母，誤。原表亦無寒韻拼章組日以母例，但考慮元韻只拼脣牙喉，與章母拼咍韻與微韻並不衝突類似，故可拼作 jat
 * - 幫三B清入（碧）：原表視為清A合口作 pvek，與「辟」小韻開合對立。原表清韻無B類，但可據體系推出處理為清B時的拼法 pyek。此外該小韻亦可按庚三處理為 pyak。本代碼同時支持三種處理法
 */

// 可調整參數
const options = {
    清B脣音: null,  // 清B脣音（「碧」小韻）處理法：默認（`null`）為按清B拼寫
                    // 其他可選值：
                    // - `'合口'`：按清A合口拼寫，為原版《〈廣韻〉全字表》處理法
                    // - `'庚三'`：按庚三拼寫
};

// 調整音韻地位
if (音韻地位.屬於('幫組 清韻 重紐B類') && options.清B脣音 === '庚三') {
    let { 母, 聲 } = 音韻地位;
