 ...
 * - 本系統（qieyun-js）的音韻地位分析（基於 poem《廣韻字音表》）與原表有數十小韻在聲母、開合、等、重紐類等歸類有所不同，除個別外均不影響拼寫體系
...
const options = {
  ...
  脣音咍韻作開口: 1, // 脣音咍韻是否視作開口，可選值：
                     // - `0`：一律視作合口
                     // - `1`（默認）：視 `音韻地位.韻` 而定，
                     //   注意 `qieyun-js` 數據脣音均歸灰韻，
                     //   而韻典數據依《廣韻》實際分韻
                     // - `2`：一律按《廣韻》分韻（依 `字頭` 參數判斷），模仿原表處理法
  凡韻非脣音作合口: 1, // 凡韻非脣音是否視作合口，可選值：
                       // - `0`：一律視作開口
                       // - `1`（默認）：視 `音韻地位.韻` 而定，
                       //   注意 `qieyun-js` 數據非脣音均歸嚴韻，
                       //   韻典數據上、入聲按《廣韻》實際分韻定開合，而去聲非脣音均歸嚴韻（原表亦如此）
                       // - `2`：上、入聲按《廣韻》分韻（依 `字頭` 參數判斷），模仿原表處理法
                       // - `3`：四聲均按《廣韻》分韻
};
...
} else if (options.脣音咍韻作開口 === 2 && 音韻地位.屬於(`幫組 灰韻`)) {
  // 「𤗏」「挴」開合口讀音均有，無法判斷，僅取合口音
  // 「䆀」僅上聲可確定為開口，去聲則兩音均有僅取合口
  // 「脄」僅去聲為開口，平聲則為合口
  const { 母, 等, 聲 } = 音韻地位;
  if ([...'㯁𡜊啡俖朏倍菩蓓'].includes(字頭)
    || (字頭 === '䆀' && 聲 === '上')
    || (字頭 === '脄' && 聲 === '去')) {
    音韻地位 = new 音韻地位.constructor(母, null, 等, null, '咍', 聲);
  }
}
...
} else if ([2, 3].includes(options.凡韻非脣音作合口) && 音韻地位.屬於('嚴韻')) {
  const { 母, 等, 聲 } = 音韻地位;
  if ([...'凵㧄𠑆𨇧猲姂䎎𣹵𡷝𦑣'].includes(字頭)
    || (options.凡韻非脣音作合口 === 3 && [...'劒欠㐸俺㤿淹𦑎㛪裺䛳覎'].includes(字頭))) {
    音韻地位 = new 音韻地位.constructor(母, '合', 等, null, '凡', 聲);
  }
}
...
// 三等重紐、重韻
if ((is('重紐B類') && (options.清B脣音 !== '合口' || !is('幫組 清韻')))
  || (['i', 'e'].includes(元)
    && is('三等 知莊組 或 云母')
