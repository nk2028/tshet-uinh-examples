/* 柳漫廣通中古拼音
 * https://zhuanlan.zhihu.com/p/453393955
 * 廣通中古拼音方案：https://zh.wikipedia.org/wiki/User:%E6%9F%B3%E6%BC%AB/%E4%B8%AD%E5%8F%A4%E6%BC%A2%E8%AA%9E%E6%8B%BC%E9%9F%B3%EF%BC%88%E5%BB%A3%E9%80%9A%E7%BE%85%E9%A6%AC%E5%AD%97%EF%BC%89
 * @author uliloewi
 */
...
if (!音韻地位) return [
  ...
  function 韻母規則() {
    ...
    // 止攝 
    if (is('支韻 合口')) return is(SangA) ? 'yɪ' : 'ʉɪ';//皆三合
    if (is('支韻') && is('幫組 或 開口')) return is(SangA) ? 'iɪ' : 'ɨɪ';//皆三開
    ...
    if (is('宵韻')) return is(SangA) ? 'iæu' : 'ɨæu';//皆三開
    ...
    // 深攝 
    if (is('侵韻')) return is(SangA) ? 'im' : 'ɨm';//皆三開
    // 咸攝 
    ...
    if (is('鹽韻')) return is(SangA) ? 'iæm' : 'ɨæm';//皆三開
