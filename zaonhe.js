...
function 聲母規則(文讀) {
  ...
  if (is('疑母 開合中立 模韻 上聲')) return 文讀 ? '▽▽ʔ' : 'ŋ'; // 「五」。表中無
  if (is('疑母 開合中立')) return 文讀 ? '▽▽ɦ' : 'ŋ'; // 「吾」等
  if (is('疑母 合口 遇果攝 一二等')) return 文讀 ? '▽▽ɦ' : 'ŋ'; // 「臥」
  if (is('疑母 合口 麻泰韻')) return 文讀 ? '▽▽ɦ' : 'ŋ'; // 「外」「瓦」
  if (is('疑母 合口')) return 文讀 ? 'ɦ' : '▽▽ŋ';
  if (is('疑母')) return 'ŋ';
...
function 韻母規則(文讀) {
  ...
  if (is('麻韻 三等 牙喉音')) return 文讀 ? 'ie▽' : 'ia'; // 「也」等
  ...
  if (is('陽韻 入聲 舌齒音 泥來孃母')) return 分袜麦 ? 'iɑʔ' : 'iɐʔ';
