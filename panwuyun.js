...
if (!音韻地位) return [
  ...
  ['微韻寫介音|微韻\n微韻正文作 ɤi，遺漏三等介音；211 頁腳註作 ɨɤi、iɤi，據此修正', [1,
    { value: true, text: 三C介音 + 'ɤi（修正）' },
    { value: false, text: 'ɤi（原書）' },
  ], { hidden: !is2023 }],
  ['燭韻加不成音節|燭韻\n凡例第八条「中古擬音的 i、u 元音下加弧線表示爲前滑音，如 ni̯ʊk、pu̯oi。……」，但正文中三等介音僅燭韻的作 i̯，其餘均作 i', [1,
    { value: false, text: 三C介音 + 'ʊk（修正）' },
    { value: true, text: 三C介音 + '\u032Fʊk（原書）' },
  ], { hidden: !is2023 }],
];
/* 韻典網與本方案 2000 擬音不同之處：
 *
...
function get韻母() {
  ...
  if (is2023 && 韻核 === 'ʊ' && is`入聲`)
    介音 += '\u032F'; // 不成音節符號。疑似多餘，暫從原貌
  if ([...'oʊ'].includes(韻核))
    介音 += is2013 ? 'u̯' :
      介音 ? '' : is2000 ? 'u' : 'u̯';
  if (is2023 && is`凡韻`) // 2023: 49 腳註“……本手册根据音变规则，唇音字定为凡韵，拟作-ᵘam，非唇音字定为严韵，拟作-am。”
    介音 += 'u̯';
