...
function get聲母() {
  if (音韻地位.描述 === '知開三麻平' || 音韻地位.描述 === '知開二庚上' && 字頭 === '打') return 't'; // 爹、打小韻
  return {
    幫: 'p',  滂: 'ph',  並: 'b',  明: 'm',
...
function get韻母() {
  ...
  let 韻 = 音韻地位.韻;
  表達式到韻.some((pair) => { if (is(pair[0])) return 韻 = pair[1]; });
  if (音韻地位.描述 === '影開三蒸入' && '抑𡊁𢬃𡊶'.includes(字頭) || 音韻地位.描述 === '溪開三蒸平') {
    韻 = '冰'; // 抑、硱小韻歸 B 類
  }
