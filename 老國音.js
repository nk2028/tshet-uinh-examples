...
if (!音韻地位) return [
  ...
  ...(選項.注音符號 !== false ? [
    ...
    ['介音|介音寫法', [2, {text: 'j w ɥ', value: '半元音' }, {text: 'i u y', value: '元音' }]],
    ['ㄧㄞ|「ㄧㄞ」讀音', [2, 'iæi', 'iai']], // 完
...
const 舒聲韻母規則 = () => when([
  ...
  // 蟹攝
  ['咍灰泰韻', [['開口', 'ㄞ'], ['泰韻 合口 疑母', 'ㄨㄞ'], ['', 'ㄨㄟ']]], // 泰合韻見組預測uəi但見母多uai
  ['佳皆夬韻', [['合口', 'ㄨㄞ'], ['牙喉音', 'ㄧㄞ'], ['', 'ㄞ']]], // 佳合韻牙喉音應爲/uai/，但有不少/ua/
...
const 入聲韻母規則 = () => when([
  ...
  ['痕韻', 'ㄜ'],
  ['魂韻', [['脣音', 'ㄛ'], ['', 'ㄨ']]],
  ['真韻 合口', [['知莊組', 'ㄨㄛ'], ['', 'ㄩ']]], // 知莊組uʔ、uoʔ兩者勢均，徹莊母多uʔ
  ['真臻殷韻', 'ㄧ'], // 莊組臻韻難決定取əʔ還是ɻ̍ʔ
  ['文韻', 'ㄩ'], // 見組文韻入聲讀音預測yʔ（如「屈ㄑㄩ˙」，溪文入）但見羣疑母多yeʔ
  ...
  // 宕攝
  ['唐韻', [['合口', 'ㄨㄛ'], ['', 'ㄛ']]],
  ['陽韻', [['合口 或 幫莊組', 'ㄩㄛ'], ['', 'ㄧㄛ']]], // 明陽入無字；孃陽入無常用字；莊組開口oʔ、uoʔ兩者勢均，但對應陽聲韻今讀合口呼
...
if (韻母[0] === 'ㄧ' && 聲母 === 'ㄫ') 聲母 = is`(齊之尤蒸陽韻 非 合口) 或 (侵仙先元庚韻 入聲)` ? 'ㄬ' : ''; // 要不要括著？
if (韻母[0] === 'ㄩ' && 聲母 === 'ㄫ') 聲母 = '';
...
if (韻母 === 'ɻ̍' && ['ts', 'tsʰ', 's'].includes(聲母)) 韻母 = 'ɹ̩';

// 音標風格
if (選項.注音符號 !== true && 選項.ㄬㄭ === 'ȵ ɿ ʅ') { // ㄬㄭ
  if (聲母 === 'ɲ') 聲母 = 'ȵ';
  if (韻母 === 'ɹ̩') 韻母 = 'ɿ';
  if (韻母 === 'ɻ̍') 韻母 = 'ʅ';
}
if (選項.注音符號 !== true && 韻母 === 'iai') 韻母 = 選項.ㄧㄞ; // ㄧㄞ
if (選項.注音符號 !== true && 韻母 === 'uən') 韻母 = 選項.ㄨㄣ; // ㄨㄣ
if (選項.注音符號 !== true && 選項.喉塞 === true && is`入聲`) 韻母 = 韻母 + 'ʔ'; // 喉塞音韻尾
