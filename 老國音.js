...
if (!音韻地位) return [
  ['標調方式', [5, '調值', '調線', '調符', '調號', '附標']],
  ['聲調在前\n標調方式爲「附標」時無效', false],
  ['常母平聲陰聲韻聲母和船母平聲聲母', [2, 'tʂʰ', 'ʂ']],
  ['介母|以半元音而非元音符號表示介母', false],
...
if (選項.標調方式 !== '附標') {
  if (選項.聲調在前 === true) return 聲調 + 聲母 + 韻母;
  else return 聲母 + 韻母 + 聲調;
}
if (選項.標調方式 === '附標' && is`入聲`) return 聲母 + 韻母;
return 聲母 + (聲調 ? 韻母.replace(/.*[aoəe]|.*[iɿʅuy]/, '$&' + ' ̄́̌̀'[聲調]) : 韻母);
