...
if (!音韻地位) return [
  ['文白讀', [3, '文上白下', '僅白讀', '僅文讀']],
  ['音標', [1, '擴展客家語拼音方案 (K++)', '國際音標 (IPA)']],
  ['標調方式', [1, '數字調號', '數字調值', '折線調值']]
];
...
function 韻母規則(白讀) {
  ...
  // 遇攝
  if (is('魚韻 來母')) return 'u';
  if (is('虞韻 來母')) return 'i';
  if (is('虞韻 幫組')) return 'u';
  if (is('魚虞韻 孃日曉母')) return 'i';
...
const 白讀音韻母 = 韻母處理(韻母規則(true));
const 文讀音韻母 = 韻母處理(韻母規則(false));

let 白讀音 = {
  韻母: 白讀音韻母,
  聲母: 聲母處理(聲母規則(true), 白讀音韻母),
  聲調: 聲調規則(true)
};
let 文讀音 = {
  韻母: 文讀音韻母,
  聲母: 聲母處理(聲母規則(false), 文讀音韻母),
  聲調: 聲調規則(false)
};

// K++ 轉 IPA
function 擴展客拼轉IPA(讀音){
  const 轉換結果 = {
    韻母: '',
    聲母: '',
    聲調: ''
...
if (選項.標調方式 === '數字調值' || 選項.標調方式 === '折線調值') {
  白讀音 = 轉換標調方式(白讀音, 選項.標調方式);
  文讀音 = 轉換標調方式(文讀音, 選項.標調方式);
}

const 白讀音結果 = 白讀音.聲母 + 白讀音.韻母 + 白讀音.聲調;
const 文讀音結果 = 文讀音.聲母 + 文讀音.韻母 + 文讀音.聲調;
