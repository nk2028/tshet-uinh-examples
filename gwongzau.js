...
if (!音韻地位) return [
  ['精清母', [1, '敎院：dz、ts', '語會：z、c']],
  ['諸韻', [1, '敎院：y', '語會：yu']],
  //['分œɵ', [1, '敎院：不分', '語會：分']],//暫時保留
  ['入聲調號', [1, '敎院：789', '語會：136']],
];
...
if (選項.諸韻 === '敎院：y') {
  if (韻母.startsWith('y')) 韻母 = 韻母.replace('u','');
}
//if (選項.分œɵ === '敎院：不分') {//暫時保留
//  if (韻母 === 'eoi') 韻母 ='oey';//暫時保留
//  if (韻母.startsWith('eo')) 韻母 = 韻母.replace('eo', 'oe');//暫時保留
//}//暫時保留
if (選項.入聲調號 === '語會：136') {
  if (聲調 === '7') 聲調 = '1';
  if (聲調 === '8') 聲調 = '3';
