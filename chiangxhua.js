...
if (!音韻地位) return [
  ...
  // 以下爲高級選項。如需使用，請自行去除注釋
  // ['次濁上省略喉化', true],
  // ['全濁平省略送氣', true],
  // ['部分蟹攝二等入假攝', true],
  // ['部分流攝脣音入遇攝', true],
  // ['uəC簡寫作uC', false],
  // ['山攝二等脣音歸合口', true], // 圖中“八”在合口，但二等不應該在合口。《韻鏡》《七音略》脣音刪舒、山入在合口，刪入、山舒在開口
];
...
function get等() {
  return when([
    ['以母 或 重紐A類 或 鈍音 清幽韻', '四'], // 幽韻按韻圖而非反切，一律歸四等
    ['幫組 三等', [ // 輕脣化的韻
      ['微廢韻', '四'],
      ['元凡韻', '二'],
      ['東鍾虞文歌陽尤韻', '一'],
...
function get聲調() {
  return when([
    ['平聲', { 附加符號: '̀', 調類數字: '¹' }],
    [選項.全濁上歸去 !== false ? '全濁 上聲 或 去聲' :
      '去聲', { 附加符號: '̌', 調類數字: '³' }],
    ['上聲', { 附加符號: '́', 調類數字: '²' }],
    ['入聲', { 附加符號: '', 調類數字: '⁴' }], // 入聲已由韻尾表明，無需附加符號
  ], '無聲調規則')[選項.聲調];
}
...
function 音位to音值(音節) {
  ...
  if (音節.介音.includes('ʕ')) {
    ...
    // 一等
    替換韻核('ɯ', ['ɯ', '']);
    替換韻核('ə', ['ə', ''], 選項.uəC簡寫作uC !== false && !is收ŋ韻); // [uəŋ] 不寫作 [uŋ]，以免與通攝混淆
    替換韻核('ɑ', ['ʌ', 'ɔ'], !音節.韻尾);
    替換韻核('ɑ', ['ɑ', 'ɔ'], !is收ŋ韻);
