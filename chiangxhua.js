 ...
 *
 * 説明
 * 以下內容為生成推導《聲音唱和圖》音系的函數體
 * 函數接受音韻地位，返回對應的推導《聲音唱和圖》音系
 */

// 代碼中的音韻表達式使用了全角半角空格用於對齊，需要進行移除
const is = (x) => 音韻地位.屬於(x.replace(/ +/g, ' ').replace(/　/g,'').trim());

// 如果只需要顯示音位或音值中的一個，請將另一個改為 false
const 顯示音位 = true;
const 顯示音值 = true;
const 聲調用調類表示 = false; // 打開則用 ¹²³⁴ 表示（分別為平上去入），關閉則用國際音標的附加符號表示
const 省略唇音後的w介音 = true; // 僅用於音值。打開則“八”/pʕʷɐ/ -> [pa]，關閉則 /pʕʷɐ/ -> [pwa]
...
if (唇化) {
  if (介音[0] === 'j') {
    ...
    介音 = is('幫組') && 省略唇音後的w介音 ? '' : 'w'; // I、II 等直接是 w 介音，但唇音后省略
  }
}
...
if (!顯示音位 && 顯示音值) return 音值;

return 音位 + ' \n[' + 音值 + ']'; // 如果兩個開關都是 false，也同時顯示音位、音值
