...
if (!音韻地位) return [
  ['顯示', [1,
    ...
    description: is半元音寫作元音 ?
      '音位分析據《聲音唱和圖》\n但止攝三四等開口 [i] 此處分析爲 /iɨi/ 而非《聲音唱和圖》中的 /iɨ/' :
      '音位分析據《聲音唱和圖》\n但止攝三四等開口 [i] 此處分析爲 /jɨj/ 而非《聲音唱和圖》中的 /jɨ/'
  }],
  ['半元音', [1, 'j ɥ w', 'i y u']],
  '聲',
  ['知照組', [2,
  ...
  ['蟹一合', [2, {
    value: 'aj', text: is半元音寫作元音 ?
      is表層 ? 'uɑi' : 'uai' :
      is表層 ? 'wɑj' : 'waj'
  }, {
    value: 'ɨj', text: is半元音寫作元音 ?
      is表層 ? 'ui' : 'uɨi' :
      is表層 ? 'uj' : 'wɨj'
  }], {
    description: is半元音寫作元音 ?
  ...
  ['章止開\n《聲音唱和圖》和《蒙古字韻》章止開同章蟹開，但當時口語不同；《中原音韻》章止開同莊止開，但北宋時是否已經相同不明\n日母也包含在內', [2,
    ...
    { value: '三ɨ', text: is半元音寫作元音 ? 'iɨ（獨立）' : 'jɨ（獨立）' },
    { value: '三ɨj', text: is表層 ? 'i（同章蟹開）' : is半元音寫作元音 ? 'iɨi（同章蟹開）' : 'jɨj（同章蟹開）' },
  ]],
  ['部分蟹攝二等入假攝', true],
  ['部分流攝脣音入遇攝', true],
  '調',
  ['聲調\n宋代調值不明，暫按唐代調值標記', [2, '五度數字', '五度符號', '附加符號', '調類數字', '省略']],
...
function 後處理(音節) {
  ...
  音節.聲母 = 音節.聲母.replace('ɰʲ', 'j');
  if (is半元音寫作元音) {
    [['j', 'i'], ['ɥ', 'y'], ['w', 'u']].forEach(([from, to]) => {
      音節.介音 = 音節.介音.replace(from, to);
      音節.韻尾 = 音節.韻尾.replace(from, to);
    });
  }
  if (音節.介音.includes('j')) 音節.聲母 = 音節.聲母.replace('j', '');
...
function get音節() {
  ...
  音節.帶調韻母 = 選項.聲調 === '附加符號' ?
    音節.介音 + 音節.韻核[0] + 音節.聲調 + 音節.韻核.slice(1) + 音節.韻尾 :
